...
check_model_quota() {
  ...
  available_capacity="$(
    jq -r \
      --arg quota_name "OpenAI.GlobalStandard.${model_name}" '
    [
      .[]
...
echo "Model availability and quota preflight passed for gpt-4o-mini and $round_robin_model_name in $location."
